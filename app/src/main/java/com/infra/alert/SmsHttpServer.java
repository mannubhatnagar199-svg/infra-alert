...
public class SmsHttpServer extends NanoHTTPD {
    ...
    public Response serve(IHTTPSession session) {
        ...
        try {
            if (uri.equals("/") || uri.equals("/operator")) return html(operatorPage());
            if (uri.equals("/admin")) {
            ...
            if (uri.equals("/api/downtimes")) return json(db.getDowntimesJson());
            if (uri.equals("/api/engineers")) return json(db.getEngineersJson(params.getOrDefault("downtime_id", "")));
            if (uri.equals("/api/logs")) return json(db.getLogsJson());
    ...
    private String adminPage() {
        return "<!DOCTYPE html><html><head><title>Admin</title>"
            ...
            + ".bp{background:#3498db;color:white}.bd{background:#e74c3c;color:white}"
            ...
            + "th{background:#f8f9fa}.ok{background:#d4edda;color:#155724;padding:10px;border-radius:5px;margin:8px 0}"
            + ".row{display:flex;gap:10px}.row>*{flex:1}</style></head><body>"
            + "<div class='hdr'><h2 style='margin:0'>Admin Panel - Infra Alert</h2><a href='/'>Operator View</a></div>"
            + "<div class='con'>"
            + "<div class='tabs'>"
            ...
            + "<input id='dn' placeholder='e.g. Network Down'>"
            + "<textarea id='dm' rows='2' placeholder='Default SMS message'></textarea>"
            + "<button class='btn bp' onclick='addDT()'>Add</button><div id='dtst'></div></div>"
            + "<div class='card'><h3>Downtime List</h3><div id='dtlist'>Loading...</div></div></div>"
            + "<div id='eng' style='display:none'>"
            + "<div class='card'><h3>Add Engineer</h3>"
            + "<div class='row'><input id='en' placeholder='Name'><input id='ep' placeholder='+91XXXXXXXXXX'></div>"
            + "<select id='edt'><option value=''>Select Downtime Type</option></select>"
            + "<button class='btn bp' onclick='addEng()'>Add</button><div id='engst'></div></div>"
            + "<div class='card'><h3>Engineers</h3><div id='englist'>Loading...</div></div></div>"
            + "<div id='log' style='display:none'>"
            + "<div class='card'><h3>SMS Logs</h3><div id='loglist'>Loading...</div></div></div>"
            + "</div>"
            + "<script>"
            ...
            + "let h='<table><tr><th>Name</th><th>Message</th><th>Action</th></tr>';"
            ...
            + "const sel=document.getElementById('edt');if(sel){sel.innerHTML='<option value=\"\">Select Downtime</option>';"
            + "data.forEach(d=>sel.innerHTML+=`<option value=\"${d.id}\">${d.name}</option>`);} })}"
            + "function loadEng(){fetch('/api/engineers').then(r=>r.json()).then(data=>{"
            + "let h='<table><tr><th>Name</th><th>Phone</th><th>Downtime</th><th>Action</th></tr>';"
            ...
            + "function delDT(id){if(confirm('Delete?'))post('/admin/delete-downtime',{id}).then(loadDT)}"
            + "function delEng(id){if(confirm('Delete?'))post('/admin/delete-engineer',{id}).then(loadEng)}"
            + "loadDT();"
            + "</script></body></html>";
    }
    ...
    private String operatorPage() {
        return "<!DOCTYPE html><html><head><title>Infra Alert</title>"
            ...
            + "<body><div class='hdr'><h2 style='margin:0'>Infra Alert</h2><a href='/admin'>Admin</a></div>"
